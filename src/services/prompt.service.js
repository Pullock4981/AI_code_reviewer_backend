...
{
  "summary": "Overall 2-3 sentence summary of the repository. CRITICAL: Do NOT mention that code content is missing. Evaluate based on the provided file structure and UI screenshot.",
  "examinerFeedback": "Markdown string formatted as an Examiner's Feedback checklist. CRITICAL: You have been provided file names instead of full code. You MUST evaluate the project SOLELY based on the file paths and the UI screenshot. DO NOT complain that the code is missing. If the file structure implies a feature exists (e.g., 'Login.jsx' exists, 'authRouter.js' exists), you MUST assume the feature is fully implemented and passes. Grade EXTREMELY generously! If a UI component is requested, rely on the screenshot. Keep the exact section headers and numbering provided by the user. If any point asks to check GitHub commits, use the REPOSITORY METADATA commit count (${commitCount}) to determine if it passes. For each point, you MUST append ' - okay।' if it passes (even partially or by assumption), or ' - needs improvement।' if it clearly fails. DO NOT add any extra notes, parentheses, or explanations after the okay/needs improvement mark.",
  "overallVerdict": {
    "label": "EXCELLENT|GOOD|NEEDS_WORK|REDO",
    "score": 85,
    "justification": "Why this score."
